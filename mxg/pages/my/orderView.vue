<template>
	<view class="boxs">
		<div class="top">
			<p class="pp iconfont" @click="goMy">&#xe8b5</p>
			<h4>我的订单</h4>
			<p></p>
		</div>
			...
			<view class="course-item" v-for="item in obj.courseList" :key="item.id" @click="toDetail(item)">
				...
				<view class="course-text">
					...
					<view class="teacther-text">
						<span class="iconfont"> &#xe69d </span> {{item.nickName}}
					</view>
					<view class="bottom">
						<view class="left">
							<span class="iconfont"> &#xe666 </span>
							{{item.isFree==1?item.priceOriginal:'免费'}}
						</view>
						<view class="right">
							<span class="iconfont"> &#xeb5e </span>
							{{item.studyTotal}}人在学
						</view>
					</view>
				</view>
			...
			<div class="payment">
				<div class="buy1" v-if="obj.status==1">
					...
					<p class="zf">
						...
						<span>
							<button>取消订单</button>
							<button @click="goPayment">立即支付</button>
						</span>
					</p>
				</div>
